...
module.exports = exports = function (socket, config) {
    ...
    function onConnection(socket) {
        ...
        socket.on('report-punchhole-status', function(status, remoteUserId, callback) {
			if (users[socket.userid] && users[remoteUserId]) {
                if (status.search(/closed|failed/gi) !== -1) {
                    console.log("status.iceConnectionState === closed|failed");
		...
		socket.on('join-broadcast', function(user) {
			try {
			    console.debug('join-broadcast - user: ' + JSON.stringify(user));
                ...
                Object.keys(users).forEach(function(uid) {
                    ...
                    try {
                        ...
                        list.push({
                            ...
                            maxRelayCountPerUser: user.maxRelayCountPerUser,
